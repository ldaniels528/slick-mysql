package com.ldaniels528.slick.mysql
...
object MySQLtoSlickGenerator {
   ...
   */
  private[slick] def generateSources(models: Seq[TableModel], outputDirectory: File): Unit = {
    var count = 0
   ...
   */
  private[slick] def generateSource(model: TableModel): String = {
   ...
   */
  private[slick] def generateColumnFunctions(columnModels: Seq[ColumnModel]): List[String] = {
   ...
   */
  private[slick] def generateForeignKeys(foreignKeys: Seq[ForeignKey]): List[String] = {
   ...
   */
  private[slick] def generateImports(columnModels: Seq[ColumnModel]): List[String] = {
   ...
   */
  private[slick] def extractTableModels(configPath: String): Seq[TableModel] = {
   ...
   */
  private[slick] def createColumnModels(columns: Seq[Column],
